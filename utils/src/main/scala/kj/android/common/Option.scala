...
final class Option[+A] private (val value: Any) extends AnyVal {
  ...
  @inline def get: A = if (isDefined) value.asInstanceOf[A] else throw new NoSuchElementException("None.get")
  ...
  @inline def filter(p: A ⇒ Boolean): Option[A] = if (isEmpty || p(unsafeGet)) this else Option.empty[A]
  @inline def filterNot(p: A ⇒ Boolean): Option[A] = if (isEmpty || !p(unsafeGet)) this else Option.empty[A]
  @inline def withFilter(f: A ⇒ Boolean): Option[A] = filter(f)

  @inline def map[B](f: A ⇒ B): Option[B] = if (isEmpty) Option.empty[B] else Option.apply[B](f(unsafeGet))
  @inline def flatMap[B](f: A ⇒ Option[B]): Option[B] = if (isEmpty) Option.empty[B] else f(unsafeGet)
  @inline def collect[B](pf: PartialFunction[A, B]): Option[B] = if (!isEmpty && pf.isDefinedAt(get)) Option.apply[B](pf(unsafeGet)) else Option.empty[B]
  ...
  @inline def flatten[B](implicit ev: A <:< Option[B]): Option[B] = if (isEmpty) Option.empty[B] else ev(unsafeGet)

  def iterator: Iterator[A] = if (isEmpty) collection.Iterator.empty else collection.Iterator.single(unsafeGet)
